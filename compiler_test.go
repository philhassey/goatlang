...
func TestCompile(t *testing.T) {
	tests := []struct {
		...
		{"var", "var x int", "ZERO int32; GLOBALSET x"},
		...
		{"varSliceMap", "var a []map[string]int", `ZERO []map[string]int32; GLOBALSET a`},
		...
		{"method", `package main; func (t *T) test(a, b int) int { return 40 + 2 }`,
			`FUNC 3:1 3 4; TYPE struct; TYPE int32; TYPE int32; TYPE int32; PUSH 40; PUSH 2; ADD; RETURN 1; GLOBALGET main.T; SETMETHOD test`},
		...
		{"localCallAttr", `func f(obj T) { obj.attr() }`, `FUNC 1:0 1 3; TYPE struct; LOCALGET $0; GETATTR attr; CALL 0 0; GLOBALFUNC f`},
		...
		{"varMap", `var x map[int]int`, `ZERO map[int32]int32; GLOBALSET x`},
		{"varObject", `type T struct {} ; var t *T`, `STRUCT 0; GLOBALSTRUCT T; ZERO struct; GLOBALSET t`},
		{"varFunc", `var x func() int`, `ZERO func; GLOBALSET x`},
		...
		{"varErr", `var err error`, `ZERO struct; GLOBALSET err`},
		{"stack", `$[0]`, `GLOBALGET $; PUSH 0; GET`},
		{"intTypes", `type T byte; var t T; t = T(42)`, `ZERO uint8; GLOBALSET t; PUSH 42; CONVERT uint8; GLOBALSET t`},
		...
		{"structArg", `func f(v *T) {}`, `FUNC 1:0 1 0; TYPE struct; GLOBALFUNC f`},
		{"structRet", `func f() *T {}`, `FUNC 0:1 0 0; TYPE struct; GLOBALFUNC f`},
		{"extStructArg", `package ext; type T struct { } ; package main; import "ext"; func f(v *ext.T) {}`, `STRUCT 0; GLOBALSTRUCT ext.T; FUNC 1:0 1 0; TYPE struct; GLOBALFUNC main.f`},
		{"extStructRet", `package ext; type T struct { } ; package main; import "ext"; func f() *ext.T {}`, `STRUCT 0; GLOBALSTRUCT ext.T; FUNC 0:1 0 0; TYPE struct; GLOBALFUNC main.f`},
		...
		{"memberInc", `type T struct { N int }; func (t *T) F() { t.N ++ }`, `GLOBALREF N; ZERO int32; STRUCT 2; GLOBALSTRUCT T; FUNC 1:0 1 5; TYPE struct; LOCALGET $0; GETATTR N; INCDEC 1; LOCALGET $0; SETATTR N; GLOBALGET T; SETMETHOD F`},
		...
		{"localStruct", `func f() any { type T struct {}; return &T{} }`, `FUNC 0:1 1 4; TYPE any; STRUCT 0; LOCALSET $0; NEWLOCALSTRUCT $0 0; RETURN 1; GLOBALFUNC f`},
		{"localStructNotGlobal", `type T struct {}; func f() any { type T struct {}; return &T{} }`, `STRUCT 0; GLOBALSTRUCT T; FUNC 0:1 1 4; TYPE any; STRUCT 0; LOCALSET $0; NEWLOCALSTRUCT $0 0; RETURN 1; GLOBALFUNC f`},
		{"localAliasNotGlobal", `type T string; func f() any { type T int; var x T = 42; return x }; var x T`, `FUNC 0:1 2 5; TYPE any; PUSH 42; CAST int32; LOCALSET $1; LOCALGET $1; RETURN 1; GLOBALFUNC f; ZERO string; GLOBALSET x`},
		...
		{"typeAliasStruct", `type T struct { K int }; type Matrix T ; x := Matrix{K:42}`, `GLOBALREF K; ZERO int32; STRUCT 2; GLOBALSTRUCT T; GLOBALREF K; PUSH 42; NEWSTRUCT Matrix 2; GLOBALSET x`},
		{"typeAliasEmptyStruct", `type T struct { K int }; type Matrix T ; x := Matrix{}`, `GLOBALREF K; ZERO int32; STRUCT 2; GLOBALSTRUCT T; NEWSTRUCT Matrix 0; GLOBALSET x`},
